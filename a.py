...
# ---------------- UTIL ----------------
def read_file(path):
    try: text = Path(path).read_text(errors="ignore"); print(f"[*] Файл '{path}' прочитан, размер: {len(text)} байт"); return text
    except: return ""

# ---------------- DETECTORS ----------------
def detect_urls(text): return list(set(re.findall(r'https?://[^\s\'"]+', text)))
def detect_base64(text, limit=BASE64_LIMIT):
    b64_regex = r'(?:[A-Za-z0-9+/]{20,}={0,2})'; found = re.findall(b64_regex, text)[:limit]; decoded=[]
...
def detect_hex_strings(text, limit=HEX_LIMIT):
    hex_regex = r'(?:[0-9a-fA-F]{2}){8,}'; found=re.findall(hex_regex,text)[:limit]; decoded=[]
...
def ultra_analyze_with_progress(js_paths, traffic_path):
    ...
    with tqdm(total=len(steps),desc="Прогресс анализа",ncols=100) as pbar:
        ...
        pbar.update(1)
        report["urls"]=detect_urls(combined_js_text); pbar.update(1)
