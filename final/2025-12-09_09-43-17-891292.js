...
GHomepage = {
	...
	BuildMainCapsuleItemv2: function( rgItem, strFeatureContext, nDepth )
	{
		...
		if ( rgItemData.review_summary &&  rgItemData.review_summary.reviewScore > 0 )
		{
			...
			let reviewSummaryText = 'Все обзоры пользователей';
			if ( reviewSummary['bLanguageOutlier'] && pref != 1 )
			{
				reviewSummaryText = 'Обзоры (русский язык)';
			}
			...
			const strTotalReviews = 'Обзоры (%1$s)'.replace( '%1$s', v_numberformat( reviewSummary['cReviews'] ) );
		...
		if ( rgRecommendationReasons.recommended_by_friend )
		{
			...
			var html = "<strong>\u0420\u0435\u043a\u043e\u043c\u0435\u043d\u0434\u043e\u0432\u0430\u043d\u043e<\/strong> \u0432\u0430\u0448\u0438\u043c \u0434\u0440\u0443\u0433\u043e\u043c, <span>%1$s<\/span>".replace("%1$s", nameHTML);
		...
		else
		{
			...
			else if ( rgRecommendationReasons.top_seller )
			{
				...
				$TopSellerRightCol.append( $J( '<div/>', { 'class': 'topseller_title' } ).text( 'Популярный товар' ) );

				let strTopSellersRank = GHomepage.bHasLocalizedTopSellers ? 'На <span>%1$s-м месте</span> в вашем регионе' : 'На <span>%1$s-м месте</span> во всём мире';
				...
				$el.on('hover focus', function(){
					...
					else
					{
						...
						if ( rgData['review_summary'] )
						{
							...
							var reviewSummaryText = rgData.appids ? 'Отзывы пользователей о продуктах в этом наборе:' : 'Все обзоры пользователей';
							if ( reviewSummary['bLanguageOutlier'] && pref != 1 )
							{
								reviewSummaryText = 'Обзоры (русский язык)';
							}
	...
	RenderTagBucketSection: function()
	{
		...
		for( var iTag = 0; iTag < rgTagBuckets.length && cTagBlocksShown < k_cTagBucketsToShow; iTag++ )
		{
			...
			$TitleCtn.append( $J('<div/>', { 'class': 'home_section_subtitle'} ).text( TagData.recommended ? 'Метка рекомендована на основе игр, в которые вы играете' : 'Избранная метка' ) )
			$Ctn.append( $TitleCtn );
			...
			$SeeMore.append( $J('<a/>', {'class': 'btnv6_white_transparent btn_small_tall', 'href': TagData.url } ).html( '<span>' + 'Ещё' + '</span>' ) );
			$Ctn.append( $SeeMore );
	...
	FillPagedCapsuleCarousel: function( rgCapsules, $elTarget, fnCapsule, strNavContext, nCapsules )
	{
		...
		for( var j=0; j<rgCapsules.length; j+=nCapsules )
		{
			...
			for( var k=0; k < nCapsules && k + j < rgCapsules.length; k++ )
			{
				...
				{
					const panelString = '{"onOptionsActionDescription":"\u041f\u043e\u0441\u043c\u043e\u0442\u0440\u0435\u0442\u044c \u0432\u0441\u0451","onOptionsButton":"window.location=\'%1$s\'"}';
	...
	BuildHomePageCapsule: function( item, strFeatureContext, rgCapsuleOptions, nDepth )
	{
		...
		if ( rgAppInfo && rgAppInfo.has_live_broadcast )
		{
			$CapCtn.append( $J('<div/>', {'class': 'broadcast_live_stream_icon' } ).append( 'Трансляция') );
		}
